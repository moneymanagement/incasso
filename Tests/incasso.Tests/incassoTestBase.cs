...
namespace Incasso.Tests
{
    public abstract class incassoTestBase : AbpIntegratedTestBase<incassoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected incassoTestBase()
        {
        ...
        protected void UsingDbContext(Action<IncassoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<IncassoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<IncassoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<IncassoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<IncassoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IncassoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<IncassoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IncassoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<IncassoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<IncassoDbContext, Task<T>> func)
        {
            T result;
