...
using Outsourcing.Web.Controllers.Results;
using Outsourcing.Web.Models;
using Outsourcing.Web.Models.Account;
using Incasso.Sessions;
...
namespace Outsourcing.Web.Controllers
{
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            if (!string.IsNullOrEmpty(loginModel.Language))
            {
                // Modify current thread's cultures            
                Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo(loginModel.Language);
            ...
            return Redirect(returnUrl); //Json(new AjaxResponse { TargetUrl = returnUrl });
        }

        private async Task<AbpLoginResultType> GetLoginResultAsync(string usernameOrEmailAddress, string password, string tenancyName)
        {
        ...
        private async Task SignInAsync(User user, ClaimsIdentity identity = null, bool rememberMe = false)
        {
            ...
            if (rememberMe)
            {
                _authenticationManager.SignIn(new AuthenticationProperties { IsPersistent = true, ExpiresUtc = DateTime.Now.AddDays(1).ToUniversalTime() }, identity);
            }
            else
            {
